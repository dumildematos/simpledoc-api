...
public interface TeamRepository extends JpaRepository<Team, Long> {
    ...
    @Query(value = "select distinct  * from team t  join invited_team it join contributor cnt on t.id = it.team_id and cnt.username = ?1 and t.name like concat('%',?2, '%') group by t.id", nativeQuery = true)
